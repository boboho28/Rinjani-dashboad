...
export const DashboardResultView: React.FC<DashboardResultViewProps> = ({
  ...
  const [formJamTutup, setFormJamTutup] = useState<string>('18:00 WIB');
  const [formJamResult, setFormJamResult] = useState<string>('18:30 WIB');
          ...
          <div className="bg-[#0d1222] border-2 border-[#ccff00]/60 rounded-3xl w-full max-w-lg p-6 shadow-[0_0_40px_rgba(204,255,0,0.3)] space-y-4">
            ...
            <form onSubmit={handleSavePasaran} className="space-y-4 text-xs font-mono-code">
              <div>
                ...
                <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="e.g. TOTOMACAU SORE" className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] font-bold outline-none" required />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={formJamTutup} onChange={(e) => setFormJamTutup(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none" />
                </div>
                <div>
                  <label className="block text-slate-400 font-bold mb-1">JAM RESULT</label>
                  <input type="text" value={formJamResult} onChange={(e) => setFormJamResult(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none" />
                </div>
              </div>

              <div>
                <label className="block text-slate-400 font-bold mb-1">LINK LIVE DRAW (Multi-Link)</label>
                <textarea rows={2} value={formLinkUrl} onChange={(e) => setFormLinkUrl(e.target.value)} placeholder="https://..." className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none resize-none" />
              </div>
