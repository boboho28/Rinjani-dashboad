...
export const DashboardResultView: React.FC<DashboardResultViewProps> = ({
  ...
  return (
    <div className="space-y-5 font-sans text-slate-100">
      ...
      {/* 1. CSS STYLES FOR ALL LOADERS, BUTTON 68, AND EFFECTS */}
      <style>{`
        ...
        /* BUTTON 68 STYLE FOR STATUS COLS */
        .btn-68 {
            position: relative;
            border-radius: 5px;
            padding: 8px 18px;
        ...
        .btn-68::before {
            ...
            animation: animation_68 3s linear infinite;
        }
        .btn-68::after {
            content: "";
            position: absolute;
        ...
        @keyframes animation_68 {
            0% { transform: rotate(0deg); }
            100% { transform: rotate(360deg); }
        }

        /* Color Variations for Button 68 */
        .btn-status-done::after { background-color: #059669; } /* Emerald */
        .btn-status-belum::after { background-color: #e11d48; } /* Rose */
        .btn-status-libur::after { background-color: #d97706; } /* Amber */
        .btn-status-now::after { background-color: #9333ea; } /* Purple */
        .btn-status-timer::after { background-color: #0891b2; } /* Cyan */
      `}</style>
        ...
        <table className="w-full text-left border-collapse min-w-[1100px] relative">
          ...
          <tbody className="divide-y divide-[#ccff00]/10 text-xs font-mono-code">
            {sortedFilteredList.map((item) => (
              <tr key={item.id} className={`hover:bg-[#ccff00]/5 transition-all group ${item.status === 'DONE' ? 'bg-[#ccff00]/5 font-bold' : item.status === 'LIBUR' ? 'bg-amber-950/20' : ''}`}>
                ...
                <td className="py-2.5 px-3">
                  <div className="relative flex items-center h-7 w-fit bg-[#0a0518] rounded-md border border-[#8b5cf6]/50 overflow-hidden shadow-[0_0_8px_rgba(139,92,246,0.3)]">
                    <div className="flex items-center justify-center h-full aspect-square bg-gradient-to-b from-[#8b5cf6] to-[#4c1d95]"><Zap className="w-3 h-3 text-white fill-white/20 animate-pulse" /></div>
                    <div className="px-3 h-full flex items-center bg-gradient-to-r from-[#1e0a3d] to-[#0a0518]"><span className="text-[10px] font-black text-purple-100 italic tracking-tighter uppercase drop-shadow-[0_0_4px_rgba(167,139,250,0.8)] font-heading">{item.session}</span></div>
                ...
                {/* NAMA PASARAN - CYBER 3D */}
                <td className="py-2.5 px-3">
                  <span className="font-brand font-black italic uppercase text-[14px] tracking-tighter text-[#22d3ee] [text-shadow:1px_1px_0_#9333ea,3px_3px_0_#4c1d95,0_0_15px_rgba(34,211,238,0.7)] group-hover:scale-110 transition-transform inline-block">{item.name}</span>
                </td>

                {/* JAM COLUMNS - DIGITAL CLOCK */}
                ...
                {/* LINK - EYE LOADER */}
                <td className="py-2.5 px-3 text-center">{item.linkUrl ? <div className="flex items-center justify-center"><button type="button" onClick={() => handleOpenAllLinks(item)} className="link-loader" /></div> : <span className="text-slate-600">-</span>}</td>

                {/* RESULT STATUS - BUTTON 68 */}
                <td className="py-2.5 px-3 text-center">
                  {(() => {
                    if (item.status === 'DONE') return <div className="btn-68 btn-status-done"><span>SUDAH RESULT</span></div>;
                    if (item.status === 'LIBUR') return <div className="btn-68 btn-status-libur"><span>PASARAN LIBUR</span></div>;
                    const match = item.jamTutup.match(/(\d{1,2}):(\d{2})/);
                    if (!match) return <div className="btn-68 btn-status-belum"><span>BELUM</span></div>;
                    const hT = parseInt(match[1], 10), mT = parseInt(match[2], 10);
                    const nowSecs = currentTime.getHours()*3600 + currentTime.getMinutes()*60 + currentTime.getSeconds();
                    const tutupSecs = hT*3600 + mT*60;
                    let diff = tutupSecs - nowSecs;
                    if (diff > 0) {
                      const m = Math.floor(diff/60), s = diff%60;
                      return <div className="btn-68 btn-status-timer"><span>⏳ {m.toString().padStart(2,'0')}:{s.toString().padStart(2,'0')}</span></div>;
                    }
                    return <div className="btn-68 btn-status-now"><span>RESULT NOW!</span></div>;
                  })()}
                </td>
                ...
                {/* STATUS - BUTTON 68 */}
                <td className="py-2.5 px-3 text-center">
                  <div onClick={() => handleCycleStatus(item)} className={`btn-68 cursor-pointer active:scale-95 transition-all ${item.status==='DONE'?'btn-status-done':item.status==='BELUM'?'btn-status-belum':'btn-status-libur'}`}>
                    <span>{item.status}</span>
                  </div>
                </td>

                {/* OPSI */}
                <td className="py-2.5 px-3 text-right"><div className="flex items-center justify-end gap-1.5"><button onClick={()=>handleOpenResultPopup(item)} className="p-1.5 bg-[#0d0f1a] border border-[#ccff00]/40 text-[#ccff00] rounded-lg hover:bg-[#ccff00]/20 transition-all"><Percent className="w-3.5 h-3.5" /></button><button onClick={()=>handleOpenEditModal(item)} className="p-1.5 bg-[#0d0f1a] border border-[#ccff00]/40 text-[#ccff00] rounded-lg hover:bg-[#ccff00]/20 transition-all"><Edit2 className="w-3.5 h-3.5" /></button><button onClick={()=>handleDeletePasaran(item.id, item.name)} className="p-1.5 bg-rose-950/80 border border-rose-500/40 text-rose-400 rounded-lg hover:bg-rose-900 transition-all"><Trash2 className="w-3.5 h-3.5" /></button></div></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0d1222] border-2 border-[#ccff00]/60 rounded-2xl w-full max-w-lg p-5 shadow-[0_0_40px_rgba(204,255,0,0.3)] space-y-4">
            <div className="flex items-center justify-between border-b border-[#ccff00]/30 pb-3"><h3 className="text-base font-black text-[#ccff00] font-brand uppercase tracking-wider">{editItem ? `EDIT - ${editItem.name}` : 'TAMBAH PASARAN'}</h3><button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-white text-lg font-bold">✕</button></div>
            <form onSubmit={handleSavePasaran} className="space-y-3 text-xs font-mono-code">
              <div><label className="block text-slate-300 font-bold mb-1 uppercase">NAMA PASARAN</label><input type="text" required value={formName} onChange={(e) => setFormName(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] font-bold outline-none" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">SHIFT</label><select value={formSession} onChange={(e) => setFormSession(e.target.value as any)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none"><option value="SORE">SORE</option><option value="PAGI">PAGI</option><option value="MALAM">MALAM</option></select></div>
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">STATUS</label><select value={formStatus} onChange={(e) => setFormStatus(e.target.value as any)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none"><option value="BELUM">BELUM</option><option value="DONE">DONE</option><option value="LIBUR">LIBUR</option></select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">JAM TUTUP</label><input type="text" value={formJamTutup} onChange={(e) => setFormJamTutup(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none" /></div>
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">JAM RESULT</label><input type="text" value={formJamResult} onChange={(e) => setFormJamResult(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none" /></div>
              </div>
              <div><label className="block text-slate-300 font-bold mb-1 uppercase">LINK DRAW</label><textarea rows={3} value={formLinkUrl} onChange={(e) => setFormLinkUrl(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/40 rounded-xl px-3 py-2 text-[#ccff00] outline-none resize-none" /></div>
              <div className="grid grid-cols-3 gap-2">
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">P1</label><input type="text" value={formP1Prize} onChange={(e) => setFormP1Prize(e.target.value)} className="w-full bg-[#141b2d] border border-[#ccff00]/50 rounded-xl px-2 py-2 text-[#ccff00] font-bold text-center outline-none" /></div>
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">P2</label><input type="text" value={formP2Prize} onChange={(e) => setFormP2Prize(e.target.value)} className="w-full bg-[#141b2d] border border-slate-700 rounded-xl px-2 py-2 text-slate-200 font-bold text-center outline-none" /></div>
                <div><label className="block text-slate-300 font-bold mb-1 uppercase">P3</label><input type="text" value={formP3Prize} onChange={(e) => setFormP3Prize(e.target.value)} className="w-full bg-[#141b2d] border border-slate-700 rounded-xl px-2 py-2 text-slate-200 font-bold text-center outline-none" /></div>
              </div>
              <div className="pt-2 flex justify-end gap-2 border-t border-[#ccff00]/30"><button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-slate-800 text-slate-300 rounded-xl font-bold font-heading uppercase">Batal</button><button type="submit" className="px-5 py-2 bg-[#ccff00] text-slate-950 rounded-xl font-black uppercase shadow-[0_0_15px_rgba(204,255,0,0.5)] font-heading">Simpan</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {activeAlarm && (
        <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
          <div className="relative w-full max-w-2xl bg-gradient-to-b from-[#141a0d] to-[#05060a] border-2 border-[#ccff00] rounded-3xl p-6 sm:p-10 shadow-[0_0_60px_rgba(204,255,0,0.45)] text-center space-y-6">
            <div className="inline-flex items-center gap-2 text-xs font-black uppercase text-[#ccff00] drop-shadow-[0_0_12px_rgba(204,255,0,0.8)]"><span className="text-xl animate-bounce">⏰</span><span>RINJANI ALARM RESULT</span></div>
            <div className="space-y-3"><h2 className="text-3xl sm:text-5xl font-brand font-black text-white tracking-wider uppercase">{activeAlarm.title || `RESULT ${activeAlarm.pasaranName}`}</h2><div className="text-lg sm:text-2xl font-brand font-black text-[#ccff00] tracking-widest uppercase">JAM RESULT {activeAlarm.jamResult}</div></div>
            <div className="pt-2"><button onClick={handleDismissAlarm} className="bg-[#ccff00] hover:bg-[#e5ff80] text-slate-950 text-base sm:text-xl font-heading font-black px-12 py-3.5 rounded-2xl shadow-[0_0_30px_rgba(204,255,0,0.75)] uppercase transition-all transform active:scale-95 cursor-pointer">OK / TUTUP</button></div>
          </div>
        </div>
      )}
      ...
      {isResultPopupOpen && popupPasaran && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0b0e1b] border-2 border-[#ccff00]/70 rounded-2xl w-full max-md sm:max-w-lg p-5 sm:p-6 shadow-[0_0_40px_rgba(204,255,0,0.25)] space-y-5 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-[#1c223a] pb-3.5"><div className="flex items-center gap-2.5"><Sparkles className="w-5 h-5 text-[#ccff00]" /><div><h3 className="text-base font-black text-[#ccff00] font-brand uppercase">HASIL RESULT & SHIO</h3><p className="text-xs text-slate-400">{popupPasaran.name}</p></div></div><button onClick={() => setIsResultPopupOpen(false)} className="p-1.5 text-slate-400 hover:text-white rounded-xl hover:bg-slate-800 transition-colors"><X className="w-5 h-5" /></button></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#12162a] border border-[#232a48] rounded-xl p-3.5 flex flex-col justify-between"><span className="text-[10px] font-bold text-slate-400 uppercase font-mono-code">RESULT P1</span><div className="text-xl font-black text-[#ccff00] tracking-widest font-mono-code mt-1">{popupPasaran.p1Prize || '-'}</div></div>
              <div className="bg-[#12162a] border border-[#232a48] rounded-xl p-3.5 flex flex-col justify-between"><span className="text-[10px] font-bold text-slate-400 uppercase font-mono-code">SHIO</span><div className="flex items-center gap-2 mt-1"><span className="text-2xl">{calculateShio(popupPasaran.p1Prize).emoji}</span><span className="text-lg font-black text-white font-heading uppercase">{calculateShio(popupPasaran.p1Prize).name}</span></div></div>
            </div>
      ...
      {showAlarmConfigModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0b0f1a] border-2 border-[#ccff00]/60 rounded-2xl w-full max-w-md p-5 shadow-[0_0_40px_rgba(204,255,0,0.3)] space-y-4">
            <div className="flex items-center justify-between border-b border-[#ccff00]/30 pb-3"><div className="flex items-center gap-2"><AlarmClock className="w-5 h-5 text-[#ccff00]" /><h3 className="text-base font-black text-[#ccff00] font-brand uppercase tracking-wider">KONFIGURASI ALARM</h3></div><button onClick={() => setShowAlarmConfigModal(false)} className="text-slate-400 hover:text-white p-1"><X className="w-5 h-5" /></button></div>
            <div className="space-y-4 text-xs font-mono-code">
              <div className="flex items-center justify-between bg-[#121624] p-3 rounded-xl border border-[#ccff00]/30"><div><div className="text-white font-bold font-heading uppercase text-xs">Otomatis Popup Alarm</div></div><input type="checkbox" checked={isAlarmEnabled} onChange={(e) => setIsAlarmEnabled(e.target.checked)} className="w-5 h-5 accent-[#ccff00] cursor-pointer" /></div>
              <div className="flex items-center justify-between bg-[#121624] p-3 rounded-xl border border-[#ccff00]/30"><div><div className="text-white font-bold font-heading uppercase text-xs">Suara Sirine</div></div><button onClick={() => setIsMuted(!isMuted)} className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 ${isMuted ? 'bg-rose-950 text-rose-400 border border-rose-500/50' : 'bg-[#ccff00]/20 text-[#ccff00] border border-[#ccff00]/50'}`}>{isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}<span>{isMuted ? 'MUTE' : 'UNMUTE'}</span></button></div>
              <button onClick={() => { setShowAlarmConfigModal(false); triggerAlarm({ pasaranName: 'TEST', jamTutup: '00:00', jamResult: '00:00', session: 'SORE', title: 'TEST OK' }); }} className="w-full bg-[#ccff00] text-slate-950 font-black py-3 rounded-xl transition-all uppercase font-heading"><Play className="w-4 h-4 fill-slate-950" /><span>TEST ALARM</span></button>
            </div>
          </div>
        </div>
      )}
